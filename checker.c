...
unsigned int stringHash(void *s){
  char *string = (char *) s;
  unsigned int hashKey = 5381;
  while (*string)
    hashKey = 33 * hashKey ^ (unsigned char) *string++;
  return hashKey;
  //Hash function taken from jd bernstein
}
...
void readDictionary(char *filename){
  //Taken and modified from tutorials point.com
  FILE *file;
  char *key;
  int wordSize = 70;
    ...
    exit(0);
     ...
     while((c = getc(file)) != EOF) {
      ...
      if (c == '\n') { //checks for newline character
        ...
        index = 0;
      } else { //for every non newline character
        word[index] = c;
        index++;
      }
     }
...
void processInput(){
   ...
   while ((ch = getchar()) != EOF) {
      ...
      if (index == maxSize) { //resize my word array
        maxSize = maxSize * 2;
        word = realloc(word, maxSize * sizeof(char));
      }

      if (isalpha(ch) == 0 && checker == 1) { //first instance of noncharacter

        //check 3 cases in dictionary

        if (findData(dictionary, word) == 0) {
          int i = 1;
          for (; i < maxSize; i++) {
        ...
        checker = 0;
        index = 0;
        //restore previous memory size
        maxSize = 70;
        //free up heap memory
        free(word);
        word = (char *) calloc(maxSize, sizeof(char));
      } 
      if (isalpha(ch)){ //alpha character
        word[index] = ch;
        checker = 1;
        index++;
      }
      printf("%c", ch); //just reguraly outputs the character

   }
   if (ch == EOF && word[0] != 0) { //Checks the last possible word before newline character
        if (findData(dictionary, word) == 0) {
          int j = 1;
          for ( ; j < maxSize; j++) {
   ...
   //Citation: http://stackoverflow.com/questions/16870485/how-can-i-read-an-input-string-of-unknown-length
   //used this to double the size of my array and realloc memory
}
